(function() {
  ...
  function d3_dsv(delimiter, mimeType) {
    ...
    var reParse = new RegExp(rn[ + delimiter + rn], g), reFormat = new RegExp('[' + delimiter + n]), delimiterCode = delimiter.charCodeAt(0);
    ...
    dsv.format = function(rows) {
      return rows.map(formatRow).join(n);
    };
    return dsv;
  }
  ...
  d3.json = function(url, callback) {
    d3.text(url, applicationjson, function(text) {
      callback(text  JSON.parse(text)  null);
    });
  };
  d3.html = function(url, callback) {
    d3.text(url, texthtml, function(text) {
      if (text != null) {
  ...
  var d3_nsPrefix = {
    svg httpwww.w3.org2000svg,
    xhtml httpwww.w3.org1999xhtml,
    xlink httpwww.w3.org1999xlink,
    xml httpwww.w3.orgXML1998namespace,
    xmlns httpwww.w3.org2000xmlns
  };
  d3.ns = {
    prefix d3_nsPrefix,
  ...
  d3.svg.brush = function() {
    ...
    function redrawX(g) {
      ...
      g.selectAll(.extent,.nrect,.srect).attr(width, extent[1][0] - extent[0][0]);
    }
    function redrawY(g) {
      g.select(.extent).attr(y, extent[0][1]);
      g.selectAll(.extent,.erect,.wrect).attr(height, extent[1][1] - extent[0][1]);
    }
    function brushstart() {
      function mouse() {
      ...
      var target = this, eventTarget = d3.select(d3.event.target), event_ = event.of(target, arguments), g = d3.select(target), resizing = eventTarget.datum(), resizingX = !^(ns)$.test(resizing) && x, resizingY = !^(ew)$.test(resizing) && y, dragging = eventTarget.classed(extent), center, origin = mouse(), offset;
  ...
  d3.csv = d3_dsv(,, textcsv);
  d3.tsv = d3_dsv(	, texttab-separated-values);
  d3.geo = {};
